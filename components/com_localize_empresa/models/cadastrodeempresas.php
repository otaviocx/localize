...
if (!JFactory::getUser()->authorise('core.edit.state', 'com_localize_empresa'))
{
	$query->where('a.state = 1');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.nome LIKE '.$search.'  OR  a.cnpj LIKE '.$search.'  OR  a.telefone1 LIKE '.$search.' )');
			}
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_LOCALIZE_EMPRESA_SEARCH_FILTER_DATE_FORMAT"), "warning");
